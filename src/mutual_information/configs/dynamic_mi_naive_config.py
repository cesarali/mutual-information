...
from mutual_information.trainers.mi_trainer_config import MITrainerConfig, DynamicNaiveMITrainerConfig
...
all_dataloaders_configs = {"ContrastiveMultivariateGaussianLoader":ContrastiveMultivariateGaussianLoaderConfig,
                           "CorrelationCoefficientGaussianLoaderConfig":CorrelationCoefficientGaussianLoaderConfig}
all_trainers_configs = {"MutualInformationTrainer":MITrainerConfig,
                        "DynamicMutualInformationTrainerNaive":DynamicNaiveMITrainerConfig}
...
@dataclass
class DynamicMutualInformationNaiveConfig:

    config_path : str = ""

    # files, directories and naming ---------------------------------------------
    delete :bool = True
    experiment_name :str = 'dynamic_mi_naive'
    experiment_type :str = 'correlated_gaussian'
...
def get_config_from_file(experiment_name, experiment_type, experiment_indentifier) -> DynamicMutualInformationNaiveConfig:
    ...
    if results_dir_path.exists():
        ...
        config = DynamicMutualInformationNaiveConfig(**config_path_json)
        return config
    else:
